...
export async function loginUser(
  ...
  try {
    ...
    return { success: true, user, wallpaper: null }
  } catch (err) {
    if (err instanceof ApiError) {
      if (err.status === 401) {
        return { success: false, user: null, wallpaper: null, error: "Noto'g'ri parol" }
      }
      if (err.status === 404) {
        return { success: false, user: null, wallpaper: null, error: 'Foydalanuvchi topilmadi' }
      }
      return { success: false, user: null, wallpaper: null, error: err.detail }
    }
    return { success: false, user: null, wallpaper: null, error: 'Tarmoq xatosi' }
  }
}
